...
func utlsIdToSpec(id ClientHelloID) (ClientHelloSpec, error) {
	...
	case HelloChrome_133.Str():
		return ClientHelloSpec{
			...
			Extensions: ShuffleChromeTLSExtensions([]TLSExtension{
				...
				&SessionTicketExtension{},
				// Chrome 133 ALPN + h3 для QUIC-совместимости
	...
	case HelloChrome_133_QUIC.Str():
		// Chrome 133 spec adapted for QUIC (h3 ALPN, no ALPS/ApplicationSettings)
		return ClientHelloSpec{
			CipherSuites: []uint16{
				GREASE_PLACEHOLDER,
				TLS_AES_128_GCM_SHA256,
			...
			Extensions: ShuffleChromeTLSExtensions([]TLSExtension{
				...
				// QUIC: нет ApplicationSettingsExtension (h2-only)
				BoringGREASEECH(),
				&UtlsGREASEExtension{},
			}),
		}, nil
